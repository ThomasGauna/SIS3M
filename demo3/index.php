...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['regreso_id'])) {
    $id = $_POST['regreso_id'];
    $observacion = !empty($_POST['observaciones']) ? $_POST['observaciones'] : 'Sin observaciones';
    $hora_regreso = date('Y-m-d H:i:s');

    $stmt = $conn->prepare("UPDATE movimientos SET hora_regreso = ?, observaciones = ? WHERE id = ?");
    $stmt->bind_param("ssi", $hora_regreso, $observacion, $id);
    $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['vehiculo_id'], $_POST['conductor_id'], $_POST['firma'])) {
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['vehiculo_id'], $_POST['conductor_id'], $_POST['firma'])) {
        ...
        $firma_nombre = uniqid('firma_') . '.png';
        $firma_ruta = 'firmas/' . $firma_nombre;
        ...
        $stmt = $conn->prepare("INSERT INTO movimientos (vehiculo_id, conductor_id, hora_salida, firma_path) VALUES (?, ?, ?, ?)");
        $stmt->bind_param("iiss", $vehiculo_id, $conductor_id, $hora_salida, $firma_ruta);
        $stmt->execute();
    }
    header("Location: index.php");
    exit;
...
<head>
  ...
  <body>
    ...
    <section class="section section-md fondo" style="padding-top: 0px;">
          <div class="contenedor-general">
            <form id="formulario" method="POST" action="">
              <label for="vehiculo_id">Vehículo:</label><br>
              <select name="vehiculo_id" required style="width: 100%;">
                  <option value="">-- Seleccionar --</option>
                  <?php
                    $res = $conn->query("SELECT id, patente, marca, modelo FROM vehiculos");
                      while ($row = $res->fetch_assoc()) {
                          echo "<option value='{$row['id']}'>{$row['patente']} - {$row['marca']} {$row['modelo']}</option>";
                      }
                  ?>
              </select><br><br>

              <label for="conductor_id">Conductor:</label><br>
              <select name="conductor_id" required style="width: 100%;">
                  <option value="">-- Seleccionar --</option>
                  <?php
                    $res = $conn->query("SELECT id, nombre, dni FROM conductores");
                      while ($row = $res->fetch_assoc()) {
              ...
              <canvas id="canvas" style="width: 100%;"></canvas><br>
              <button type="button" onclick="limpiar()" class="btn">Limpiar Firma</button><br><br>

              <input type="hidden" name="firma" id="firma">
              <input type="submit" class="btn" value="Registrar salida">
            </form>
        </div>
    </section>
    
    <section class="section section-md fondo" style="padding-bottom: 0px; padding-top: 0px;">
     <div class="contenedor-general" style="padding: 0px;">
    ...
    <section class="section section-md fondo" style="padding-top: 0px;">
     <div class="contenedor-general">
            <table class="responsive-table" style="margin-top: 0px;">
              <thead>
                <tr>
                  <th>Marca</th>
                  <th>Modelo</th>
              ...
              <tbody>
                <?php
                  $query = "SELECT m.id, v.patente, v.marca, v.modelo, c.nombre, m.hora_salida
                            FROM movimientos m
                            JOIN vehiculos v ON m.vehiculo_id = v.id
                            JOIN conductores c ON m.conductor_id = c.id
                            WHERE m.hora_regreso IS NULL
                  ...
                  $res = $conn->query($query);
                    if ($res->num_rows > 0) {
                        ...
                        echo "<tr>
                            ...
                            <td data-label='Conductor:'>{$row['nombre']}</td>
                            <td data-label='Hora de salida:'>" . date('d-m-Y H:i', strtotime($row['hora_salida'])) . "</td>
                            <td data-label='Observación:'>
                              <form method='POST' action=''>
                                <input type='hidden' name='regreso_id' value='{$row['id']}'>
                                <input type='text' name='observaciones' placeholder='' style='width: 100%; box-sizing: border-box; margin-bottom: 5px;' value>
                            </td>
                            <td data-label='Acción'>
                                <button type='submit' class='btn'>Registrar regreso</button>
                              </form>
                            </td>
                        </tr>";
                        }
                    } else {
                        echo "<tr><td colspan='7'>No hay vehículos pendientes de regreso.</td></tr>";
                    }
                ?>
              </tbody>
            </table>
    ...
    <script src="js/registrar_movimiento.js"></script>
  </body>
</html>